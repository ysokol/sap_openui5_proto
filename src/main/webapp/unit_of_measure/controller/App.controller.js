sap.ui.define([
	...
	return Controller.extend("ys.first.app.controller.App", {
		...
		onInit: function() {
			...
			this._oItemTemplate = this.byId("columnListItemId").clone();

			this.getView().setModel(this._oModel);
			sap.ui.getCore().setModel(this._oModel);
			var binding = new sap.ui.model.Binding(this.getOwnerComponent().getModel("globalProperties"), "/", this.getOwnerComponent().getModel(
				"globalProperties").getContext("/"));
			var that = this;
			binding.attachChange(function(oEvent) {
				debugger;
				var aFilters = [];
				var oFilter = new sap.ui.model.Filter("Client", "EQ", oEvent.getSource().getModel().getProperty("/client"),
					null);
		...
		onBtnRefresh: function(oEvent) {
			...
			this.getOwnerComponent().getModel("globalProperties").setProperty("/client", "010");
		}
	});
});